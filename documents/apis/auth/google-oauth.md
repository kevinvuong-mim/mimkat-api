...
```bash
GET /auth/google?redirect_url={frontend_url}
```
...
| ------------ | ------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| redirect_url | string | No       | URL frontend để redirect về sau khi login thành công. Nếu không có, sẽ tự động extract từ request headers. Phải được encode bằng `encodeURIComponent()`. |

**Ví dụ**:

```bash
GET /auth/google?redirect_url=http%3A%2F%2Flocalhost%3A3000
# hoặc
GET /auth/google?redirect_url=https%3A%2F%2Fexample.com%2Fdashboard
```
...
1. Client gọi `/auth/google?redirect_url={frontend_url}` (hoặc click "Login with Google" button)
2. GoogleAuthGuard (Passport strategy) xử lý request:
   - Lấy `redirect_url` từ query parameter (nếu có)
   - Nếu không có, tự động extract từ request headers (sử dụng `extractFrontendUrl()`)
   - Đưa redirect URL vào `state` parameter để Google trả lại sau
3. Backend redirect đến `https://accounts.google.com/o/oauth2/v2/auth` với các parameters:
   ...
   - `response_type`: `code`
   - `scope`: `email profile`
   - `state`: `{redirect_url}` (URL frontend để redirect về sau khi login)
   - `access_type`: `offline` (optional)
4. User chọn tài khoản Google và cấp quyền
5. Google redirect về `/auth/google/callback?code={authorization_code}&state={redirect_url}`
...
| state     | string | No       | Redirect URL từ client (được truyền từ query parameter `redirect_url` khi gọi `/auth/google`). Backend sẽ redirect về URL này sau khi xử lý xong. |
...
2. Redirect về frontend URL từ `state` parameter (URL mà client đã gửi khi gọi `/auth/google?redirect_url=...`)
